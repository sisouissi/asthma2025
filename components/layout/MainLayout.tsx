...
const Header: React.FC = () => {
  ...
  return (
    <header className="bg-white/80 backdrop-blur-md sticky top-0 z-30 border-b border-slate-200/60 shadow-sm no-print py-4">
      <div className="container mx-auto max-w-7xl px-4 flex items-center justify-between gap-6">
        <div 
            className="flex items-center gap-4 cursor-pointer group" 
            onClick={resetNavigation}
        >
            <div className="bg-gradient-to-br from-indigo-500 to-violet-600 p-3 rounded-2xl shadow-lg group-hover:scale-105 transition-transform duration-200">
                <Lightbulb className="w-8 h-8 text-white" />
            </div>
            <div>
                <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 group-hover:text-indigo-700 transition-colors">GINA Asthma Guide</h1>
                <p className="text-sm sm:text-base text-slate-500 font-medium hidden sm:block">Evidence-based Management & Prevention</p>
            </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
           {currentStepId !== 'PATIENT_DASHBOARD' && (
             <Button
                ...
                variant="primary"
                size="lg"
                className="hidden sm:inline-flex shadow-indigo-100 font-semibold"
              >
                Patient Access
              </Button>
           )}
           ...
           {currentStepId !== 'PATIENT_DASHBOARD' && (
             <button 
                onClick={() => navigateTo('PATIENT_DASHBOARD')}
                className="sm:hidden p-3 bg-indigo-50 text-indigo-600 rounded-xl"
             >
                <User size={24} />
             </button>
           )}
          ...
          <Button
            ...
            // Changed to 'secondary' and added custom classes for distinction
            variant="secondary" 
            size="lg"
            className="hidden md:inline-flex font-semibold text-indigo-700 bg-indigo-50 border-indigo-200 hover:bg-indigo-100 shadow-sm"
          >
            Home
          </Button>
          ...
          {currentStepId !== 'ABBREVIATIONS_STEP' && (
            <Button
              ...
              variant="secondary" 
              size="lg"
              className="font-medium border-slate-300 text-slate-700 shadow-sm"
            >
              <span className="hidden lg:inline">Abbreviations</span>
...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="pb-12 pt-8 no-print bg-slate-50 border-t border-slate-200 mt-auto">
      <div className="container mx-auto max-w-7xl px-4 space-y-8">
        ...
        {currentPatient && currentStepId !== 'PATIENT_DASHBOARD' && currentStepId !== 'CONSULTATION_SUMMARY_STEP' && (
            <div className="bg-white border border-indigo-100 shadow-md rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-center gap-4 animate-fade-in">
                <div className="flex items-center text-slate-700">
                    <div className="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center mr-4 text-indigo-700 font-bold text-lg">
                        {currentPatient.firstName[0]}
                    </div>
                    <div>
                        <p className="text-xs text-slate-500 uppercase font-bold tracking-wider mb-0.5">Active Patient</p>
        ...
        <div className="flex flex-col-reverse md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex flex-wrap justify-center gap-3 w-full md:w-auto">
            {currentStepId !== 'INITIAL_STEP' && (
             ...
             {/* Quick Links Bar */}
             <div className="flex gap-2 bg-white p-2 rounded-xl shadow-sm border border-slate-200 overflow-x-auto max-w-full">
                <Button
                ...
                >
                 ...
                 <Button
                    ...
                    variant="ghost"
                    size="md"
                    className="text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 font-medium"
                >
                    Management
                </Button>
                <Button
                    ...
                    variant="ghost"
                    size="md"
                    className="text-slate-600 hover:text-sky-600 hover:bg-sky-50 font-medium"
                >
                    Diagnosis
                </Button>
                 <Button
        ...
        <div className="border-t border-slate-200 pt-6">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-4 text-xs text-slate-500">
                ...
                <p className="leading-relaxed max-w-4xl">
                    ...
                    All treatment decisions must be individualized.
                </p>
                <div className="ml-auto text-slate-400 whitespace-nowrap font-medium">
                    © Dr. Zouhair Souissi
                </div>
            </div>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 font-sans">
      ...
      <div className="fixed bottom-6 right-6 z-40 no-print">
        <button
            onClick={openAIPanel}
            className="group relative w-16 h-16 bg-gradient-to-br from-indigo-600 to-violet-600 rounded-2xl shadow-xl hover:shadow-indigo-500/40 hover:-translate-y-1 transition-all duration-300 flex items-center justify-center"
