...
import { Stethoscope, CheckCircle2, ChevronRight, CheckSquare, Square, AlertTriangle, TrendingUp, ClipboardList, Droplets, HelpCircle, Pill } from 'lucide-react';
...
const AdultDiagnosisStep: React.FC = () => {
  ...
  const handleToggle = (section: keyof GinaDiagnosisCriteria, field: string) => {
    if (section === 'biomarkers') return; // Handled separately
    updatePatientData({
      diagnosisCriteria: {
        ...diagnosisCriteria,
        [section]: {
  ...
  const assessment = useMemo(() => {
    ...
    if (!isOnMaintenanceTreatment) {
        ...
        const hasSupportiveBiomarkers = feNo > 50 || eos >= 300;

        if (hasSymptom && hasPattern && hasAirflowLimitation) {
            return { status: 'confirmed', message: 'Criteria Met: Diagnosis of Asthma Confirmed', color: 'emerald' };
        }
        
        if (hasSymptom && hasSupportiveBiomarkers) {
            return { 
                status: 'supported', 
                message: 'Supported by Type 2 Biomarkers (FeNO > 50ppb or Eos ≥ 300/μL)', 
                color: 'emerald',
             ...
             if (!isNaN(fev1)) {
                 if (fev1 > 70) {
                     return { 
                         ...
                         color: 'amber', 
                         detail: 'Consider stepping down controller (after checking SABA/LABA withholding) to unmask variability. Reassess in 2-4 weeks.' 
                     };
                 } else {
                     return { 
                         status: 'probable', 
  ...
  const handleContinue = () => {
    // If confirmed or strongly supported/recommended action taken, proceed
    if (assessment.status === 'confirmed' || assessment.status === 'supported') {
        ...
        // For probable/unlikely, we flag as unconfirmed but can allow proceed or route to pending
        updatePatientData({ diagnosisConfirmed: false });
        
        // If on treatment and we have a clear plan (Step Up/Down), we can arguably proceed to risk assessment 
        // effectively treating it as a 'working diagnosis' for the session
  ...
  return (
    <Card 
      title="Diagnostic Criteria Assessment" 
      icon={<Stethoscope className="text-indigo-600" />}
      titleRightElement={<span className="text-sm font-normal text-slate-500">Adults & Adolescents</span>}
    >
      ...
      <div className="mb-6 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
          ...
          <p className="text-sm text-indigo-800 mb-3">Is the patient currently taking controller medication (e.g., ICS, OCS)?</p>
          <div className="flex gap-4">
              <Button 
                onClick={() => handleTreatmentStatusChange(true)} 
                variant={isOnMaintenanceTreatment ? 'primary' : 'secondary'}
                size="sm"
              >
                  Yes, on Controller
              </Button>
      ...
      {!isOnMaintenanceTreatment ? (
          // Standard GINA Box 1-2 Flow
          <div className="space-y-6">
            <p className="text-sm text-slate-600">
                Select all that apply to determine if the patient meets the GINA criteria for initial diagnosis (GINA Box 1-2).
            </p>
            <div>
            ...
            <div>
                ...
                <CheckboxGroup 
                    ...
                    items={[
                        { key: 'bdReversibility', label: 'Positive BD Reversibility (Adult: FEV1 >12% AND >200mL)' },
                        { key: 'pefVariability', label: 'Average daily diurnal PEF variability >10%' },
                        { key: 'treatmentTrial', label: 'Significant increase in lung function after 4 weeks anti-inflammatory treatment' },
                        { key: 'challengeTest', label: 'Positive exercise or bronchial challenge test' },
                        { key: 'visitVariation', label: 'Excessive variation in FEV1 between visits (>12% and >200mL)' }
                    ]} 
                />
            </div>
          </div>
      ) : (
          // On Treatment Diagnosis Flow (GINA Box 1-4)
          <div className="space-y-6">
              <p className="text-sm text-slate-600">
                Confirming diagnosis in patients already on treatment is challenging (GINA Box 1-4). Select the patient's current status:
              </p>
        ...
        {/* 4. Supportive Biomarkers - Visible in BOTH modes */}
        <div className="mt-8 pt-6 border-t border-slate-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input 
                        type="number" 
                        className="w-full p-2 bg-slate-100 border border-slate-300 rounded-md shadow-inner text-slate-800 placeholder-slate-400 focus:bg-white focus:shadow-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200"
                        placeholder="e.g., 25"
                        value={diagnosisCriteria.biomarkers.feNo}
                        onChange={(e) => handleBiomarkerChange('feNo', e.target.value)}
                    />
                    <p className="text-[10px] text-slate-500 mt-1">&gt;50 ppb strongly supports diagnosis</p>
                </div>
                <div>
      ...
      <div className={`mt-8 p-4 rounded-lg border-l-4 bg-${assessment.color}-50 border-${assessment.color}-500 shadow-sm`}>
          <div className="flex items-start">
             ...
             <div>
                 ...
                 {assessment.status === 'probable' && !isOnMaintenanceTreatment && <p className="text-sm text-amber-700">Objective testing is required to confirm diagnosis before starting long-term treatment.</p>}
             </div>
          </div>
      </div>
      ...
      <div className="mt-8">
        ...
        >
          {assessment.status === 'confirmed' || assessment.status === 'supported' || (isOnMaintenanceTreatment && assessment.status === 'probable') ? 'Continue' : 'View Recommendations'}
        </Button>
      </div>
    </Card>
  );
