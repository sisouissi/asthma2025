...
export const PatientDataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // GINA 2025 Logic Update: 
    // Biologic eligibility requires Severe Asthma (Uncontrolled + Adherence/Technique) 
    // AND (Type 2 Inflammation OR OCS Maintenance).
    // Note: Exacerbations are a key driver, but OCS dependence is also a valid indication for OCS-sparing therapy (e.g. Anti-IL5/Anti-IL4R) even without frequent exacerbations in the prior year (as OCS may suppress them).
    // We also include 'difficultToTreat' if strict 'severeAsthma' (adherence) check fails, to allow clinicians to explore options in simulation mode.

    const isCandidates = newResults.severeAsthma || newResults.difficultToTreat;
    const hasExacerbations = (parseInt(basicInfo.exacerbationsLastYear) || 0) >= 1;

    newResults.eligibleForBiologics = isCandidates &&
      (
        (newResults.type2Inflammation && hasExacerbations) ||
        medications.maintenanceOcs
      );
